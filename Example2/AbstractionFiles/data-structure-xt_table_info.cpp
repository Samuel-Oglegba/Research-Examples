...
int main ()
{
     ...
     // memcpy(newinfo, info, offsetof(struct xt_table_info, entries));   
      memcpy(newinfo, info, sizeof(xt_table_info.entries));

      newinfo->initial_entries = 0;
      loc_cpu_entry = info->entries;

      xt_table_info.xt_compat_init_offsets(AF_INET, info->number);
                        ...
                        (iter) = (typeof(iter))((char *)(iter) + (iter)->next_offset))
      {
                  ...
                  ret = xt_table_info.compat_calc_entry(iter, info, loc_cpu_entry, newinfo);
                  if (ret != 0)
                        return ret;
