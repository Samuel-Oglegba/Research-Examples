 ...
 * 
 * Input:	@param net --  A struct of independent logical copy of the host network stack, represents the virtual container. 
 *				   It is the network namespace through which the ip router enteries check was invoked.
 * 		@param name -- The name of the table entry
 * 		@param valid_hooks --  Bitmap indicating checks to run. 
 * 					     Used to filter out the table entries to be validated, if 1 validate else ignore.
 * 		@param pinfo -- The struct representing network routing table, it is keyed by destination IP address.
 * 				    
 * 		@param pentry0 -- The routing table entries.
 * 					This are the entries that are validated
		       ...
		       unsigned int *underflows)
{
	...
	/* Walk through entries, checking offsets. */
      /**
       * @brief iteratively check to ensure that the size of the table entries & hook are correct
       * 
       */
	...
	ret = -ENOMEM;
      /**
       * @brief allocate memory to newinfo
       * 
       */
 ...
 * Output	@return int -- possible output {0 -- default operation, 1 -- for success, negative values for when something goes wrong}
 * 				   struct xt_entry_match, and xt_entry_target stores the converted data structures from 32bit to 64bit
 */
